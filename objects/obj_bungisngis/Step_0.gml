if (is_walking) {
    ...
    if (walk_timer <= 0) {
        // Passe en mode chill ✨
        is_walking = false;
        sprite_index = spr_bungisngis_bottom_walk; // Pose par défaut pendant l'arrêt
        ...
        if (_ver > 0) {
            sprite_index = spr_bungisngis_bottom_walk; // Marche vers le bas
        } else if (_ver < 0) {
            sprite_index = spr_bungisngis_top_walk; // Marche vers le haut
        } else if (_hor > 0) {
            sprite_index = spr_bungisngis_right_walk; // Marche vers la droite
        } else if (_hor < 0) {
            sprite_index = spr_bungisngis_left_walk; // Marche vers la gauche
        }
    }
} else {
    // Mode idle UwU
